...
function runMacro(p: MacroParams, quarters = 40): MacroSnap[] {
  ...
  for (let q = 0; q <= quarters; q++) {
    ...
    const rawDemand = consumption + investment + govtSpending + netExports;
    // GDP partially adjusts toward demand each quarter (sticky prices/wages)
    const adjustSpeed = 0.55;
    realGDP = realGDP + adjustSpeed * (rawDemand - realGDP);
    ...
    inflation = clamp(newInflation, -2, 30);
    ...
    debtPct = clamp(debtPct * (1 - nomGrowthQ) - balance / 4, 0, 300);
