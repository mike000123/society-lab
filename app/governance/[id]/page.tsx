...
import { ArrowLeft, ThumbsUp, ThumbsDown, BookOpen, ArrowRight, MessageSquareQuote } from "lucide-react";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { ProposalCommentsSidebar } from "@/components/governance/ProposalCommentsSidebar";
import {
  CATEGORY_META,
  getProposalById,
  type Proposal,
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_24rem] xl:items-start">
        <div className="space-y-5">
          ...
          {proposal.discussionThreadId && (
            <section className="rounded-[1.75rem] border border-slate-800 bg-panel/90 p-5 sm:p-6">
              <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Discussion provenance</p>
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-start gap-2">
                  <MessageSquareQuote className="mt-0.5 h-4 w-4 text-cyan-300" />
                  <div>
                    <p className="text-sm font-medium text-slate-200">
                      {proposal.discussionThreadTitle ?? "Discussion that informed this proposal"}
                    </p>
                    <p className="mt-1 text-sm leading-6 text-slate-400">
                      This proposal points back to the public discussion thread that helped shape it.
                    </p>
                  </div>
                </div>
                <Button asChild variant="outline" size="sm" className="rounded-2xl gap-1.5">
                  <Link href={`/discussions?thread=${proposal.discussionThreadId}`}>
                    Open discussion <ArrowRight className="h-3.5 w-3.5" />
                  </Link>
                </Button>
              </div>
            </section>
        ...
        <div className="xl:sticky xl:top-24">
          <ProposalCommentsSidebar proposalId={proposal.id} />
        </div>
      </div>
    </div>
  );
